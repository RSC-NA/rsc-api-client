...
[**tracker_links_spider_create**](TrackerLinksApi.md#tracker_links_spider_create) | **POST** /tracker-links/spider/ | 
...
async with rscapi.ApiClient(configuration) as api_client:
    ...
    active = True # bool | Restrict to trackers belonging to active members. Defaults to **true** -- historical pulls are only needed for members actually in the league. Set false to include inactive members for a backfill or audit. Trackers with no member attached are never served by `next/` either way, since a pull cannot be recorded without one. (optional) (default to True)
    format = 'format_example' # str |  (optional)
    limit = 56 # int | Number of tracker links to claim (Default: 1, clamped to 25). Returned trackers are claimed: each is stamped with `last_attempt_at`, so successive calls advance through the queue rather than returning the same rows. A claimed tracker is withheld for a cooldown period and then rotates back into its priority rung, so a tracker that cannot be pulled will not block the queue. Priority order is: never pulled, missing season history, stale peak, previously-failed attempts, then scraped-but-empty last. Trackers marked invalid by the nightly account validation are never served. Returns an empty array when nothing is due. Non-integer or < 1 values return 400. (optional)
...
# **tracker_links_spider_create**
> TrackerLink tracker_links_spider_create(tracker_link_spider_request=tracker_link_spider_request)

Resolve every Epic tracker the player holds through RL Planet and create tracker links for the accounts it reports as linked. Returns the links that were created. Existing links are never modified, and accounts already held by a different member are reported rather than created.
...
async with rscapi.ApiClient(configuration) as api_client:
    # Create an instance of the API class
    api_instance = rscapi.TrackerLinksApi(api_client)
    tracker_link_spider_request = rscapi.TrackerLinkSpiderRequest() # TrackerLinkSpiderRequest |  (optional)

    try:
        api_response = await api_instance.tracker_links_spider_create(tracker_link_spider_request=tracker_link_spider_request)
        print("The response of TrackerLinksApi->tracker_links_spider_create:\n")
        pprint(api_response)
    except Exception as e:
        print("Exception when calling TrackerLinksApi->tracker_links_spider_create: %s\n" % e)
```
...
Name | Type | Description  | Notes
------------- | ------------- | ------------- | -------------
 **tracker_link_spider_request** | [**TrackerLinkSpiderRequest**](TrackerLinkSpiderRequest.md)|  | [optional] 
